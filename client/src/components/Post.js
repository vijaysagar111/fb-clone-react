  ...
  render() {
    ...
    return (
      ...
      <div>
       ...
       <p style = {{textAlign: 'left'}} ><span style = {{fontSize: '18px', fontWeight: 'bold', color: '#4968AC'}}>{this.props.post.user.firstname} {this.props.post.user.lastname}</span>  <br></br>
        {moment(this.props.post.createdAt).format('MMMM Do YYYY LTS')}</p>
        <br></br>
      <img src= {this.props.source} className="img-fluid" style= {{width: '70%'}}/>
        {this.state.editting
          ?//if true
          ...
          <div>
            ...
            <p style={{fontSize: '20px', fontWeight: '600'}}>{this.state.value}</p>
            </div>
            {this.props.post.user._id === this.props.user._id &&
